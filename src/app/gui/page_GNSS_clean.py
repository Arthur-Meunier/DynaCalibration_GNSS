...
class GnssWidget(QWidget):
    """Widget GNSS avec gestion correcte des fichiers NAV/GNAV et navigation automatique"""
    ...
    def on_parallel_baseline_finished(self, baseline_index: int, return_code: int):
        ...
        print(f"🔍 DEBUG: on_parallel_baseline_finished appelé pour baseline {baseline_index}, code {return_code}")
        
        # Garde pour éviter les répétitions
        if baseline_index not in self.baseline_results:
            print(f"🔍 DEBUG: baseline_index {baseline_index} pas dans baseline_results, sortie")
            return
        
        current_status = self.baseline_results[baseline_index]["status"]
        if current_status in ["success", "error"]:
            print(f"🔍 DEBUG: baseline {baseline_index} déjà terminé avec statut {current_status}, sortie")
            # Déjà traité, éviter la répétition
            return
            
        baseline_name = self.rtk_calculators[baseline_index].baseline_name
        print(f"🔍 DEBUG: Traitement baseline {baseline_name}")
        ...
        print(f"🔍 DEBUG: Vérification si toutes les baselines sont terminées")
        print(f"🔍 DEBUG: Toutes terminées: {all_finished}")
        
        if all_finished:
            print("🔍 DEBUG: Appel de _on_all_parallel_baselines_finished")
    ...
    def _on_all_parallel_baselines_finished(self):
        ...
        print("🔍 DEBUG: _on_all_parallel_baselines_finished appelé")
        
        # Garde pour éviter les appels multiples
        if not hasattr(self, '_all_finished_called'):
            self._all_finished_called = True
        else:
            print("🔍 DEBUG: _on_all_parallel_baselines_finished déjà appelé, sortie")
            return  # Déjà appelé, éviter la répétition
            
        print("🔍 DEBUG: Réactivation des boutons")
        ...
        print(f"🔍 DEBUG: Succès: {successful_baselines}, Échecs: {failed_baselines}")
        ...
        # Message informatif pour la navigation
        self.log_message("📊 Navigation vers la page des statistiques dans 3 secondes...")
